...
class DockerExecutor {
  ...
  async copySourceCode(container, submission) {
    ...
    await this.writeFile(container, fileName, source_code);
  }
  ...
  async copyAdditionalFiles(container, submission) {
    ...
    try {
      ...
      await this.writeFile(container, '_additional.zip', zipBuffer);
  ...
  /**
   * Stream a file into the writable /box tmpfs. Docker's archive API rejects
   * writes when ReadonlyRootfs is enabled, even when the destination is tmpfs.
   */
  async writeFile(container, fileName, content) {
    if (!/^[a-zA-Z0-9._-]+$/.test(fileName)) {
      throw new Error(`Invalid sandbox file name: ${fileName}`);
    }
